...
class GraphDatabase:
    ...
    def _generate_builtin_cs408_triples(self):
        ...
        subjects = {
            ...
            "操作系统": ["进程", "线程", "调度", "内存管理", "文件系统", "死锁", "同步互斥", "中断"],
            "计算机网络": ["OSI", "TCP", "UDP", "IP", "路由", "拥塞控制", "HTTP", "DNS"],
            "计算机组成原理": ["数据表示", "存储系统", "CPU", "指令系统", "总线", "流水线", "中断系统", "DMA"],
        }
        triples = []
        for subject, topics in subjects.items():
            ...
            for i, topic in enumerate(topics):
                if i + 1 < len(topics):
                    triples.append({"h": topic, "r": "depends_on", "t": topics[i + 1], "subject": subject})
            for idx in range(1, 121):
                node = f"{subject}-知识点{idx:03d}"
                pivot = topics[idx % len(topics)]
                triples.append({"h": node, "r": "part_of", "t": pivot, "subject": subject})
                triples.append({"h": node, "r": "belongs_to", "t": subject, "subject": subject})
        return triples
    ...
    def _extract_json_triples(self, content: str, default_subject: str | None = None):
        ...
        triples = []
        relation_mapping = {
            "related_topics": "EQUIVALENT",
            "algorithms": "HAS_METHOD",
            "properties": "HAS_PROPERTY",
            "complexities": "HAS_COMPLEXITY",
